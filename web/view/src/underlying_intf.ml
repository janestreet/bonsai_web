...
module C = struct
  class type t = object
    ...
    (* While we strive to make the `autosize` and default variants looking the same,
       `autosize` has its own set of style sheets due to the feature requiring a different
    ...
    *)
    method prt_styling : ?autosize:bool -> unit -> For_prt.t
    ...
    method toplayer_tooltip_styles : intent:Constants.Intent.t option -> Vdom.Attr.t
    method toplayer_tooltip_anchor_styles : Vdom.Attr.t
    method toplayer_popover_styles : Vdom.Attr.t
    method toplayer_modal_styles : Vdom.Attr.t
