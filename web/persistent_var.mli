...
(** A Persistent_var.t is similar to Bonsai.Expert.Var.t, but the contents of the var are
    ...
    sexpable, so a first class module for the type should be passed in.

    You have a choice between Local Storage and Session Storage for actually persisting
    things; The docs for which can be found here:

    - https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage
    - https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage

    A [unique_id] is required for the storage. This value needs to be unique per
    application.

    Finally, a fallback is required for when the value can't be found, or it can't be
    deserialized. *)
val create
...
(** This will remove the value from localstorage or sessionstorage. (calling [set] or
    [update] afterwards will still store the value back again.) *)
