...
let make' ?(here = Stdlib.Lexing.dummy_pos) ~name bag_inst =
  if am_running_test
  then
    [%sexp
...
let make ?(here = Stdlib.Lexing.dummy_pos) ?name () = make' ~here ~name None
...
module For_bonsai_web_test = struct
  let css_selector t =
    match t with
    | "" -> ""
...
module Keyed = struct
  ...
  let create
    ...
    ?name
    (type a)
    (module M : Of_sexpable with type t = a)
    =
