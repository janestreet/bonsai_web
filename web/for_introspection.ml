...
let readModuleTimerEvents () =
  Ppx_module_timer_runtime.For_introspection.timing_events_in_reverse_chronological_order
    ()
  |> List.map ~f:For_module_startup_timings.Timing_event.Stable.of_latest
  |> [%sexp_of: For_module_startup_timings.Timing_event.Stable.t list]
  |> Sexp.to_string_mach
  |> Js.string
;;
...
class type global = object
  ...
  method ppxModuleTimerIntrospectionSupported : bool Js.t Js.prop
  ...
  method rpcEffectRevision : Js.js_string Js.t Js.optdef Js.prop
  ...
  let start_event =
    lazy
      (let query =
       ...
       in
       Event.V1.Started { id; rpc_kind; start_time; query; path; here = Some here })
  in
