...
module Arrow_deprecated : sig
  ...
  (** Start an application, receiving a handle that can schedule actions of a user-defined
      ...
      - [?profile] (defaults to false) Sends performance marks and measures
        to the browser performance timeline.*)
  val start
    :  ?optimize:bool
    -> ?simulate_body_focus_on_root_element:bool
    -> ?profile:bool
...
module Proc : sig
  ...
  [@@ocaml.doc
    ...
    \          [bind_to_element_with_id] should be the HTML id of the element in the \
     document that\n\
    \          Bonsai will take control of.  For most apps, you'll have html that looks \
     like this:\n\
    \          [<html><body><div id=\"app\"></div></body></html>], so the value passed to\n\
    \          [bind_to_element_with_id] should be the string [\"app\"].  The default \
     value of this\n\
    \          parameter is \"app\".\n\n\
    \          [custom_connector] is for expessing custom logic for connecting to RPC \
     endpoints.\n\
  ...
  [@@ocaml.doc
    ...
    \          runs it.\n\n\
    \          The first parameter to [start_and_get_handle] is a first-class module \
     that defines a\n\
    \          [view] function of type ['result -> Vdom.Node.t].  If the computation has \
     type\n\
    \          [Vdom.Node.t Bonsai.Proc.Computation.t], then \
     {!Result_spec.just_the_view} is a\n\
    \          suggested first argument.  Read the docs in {!Result_spec} for more \
     details.\n\n\
    \          [bind_to_element_with_id] should be the HTML id of the element in the \
     document that\n\
    \          Bonsai will take control of.  For most apps, you'll have html that looks \
    ...
    \          [optimize] configures Bonsai's computation optimizations, and defaults to \
     [true] "]
end
