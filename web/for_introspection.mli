...
(** [send_and_track_rpc_from_poller] sends the [send_rpc] rpc, but it also records its
    ...
    This function is specialized for "polling" functions that return a ('query, 'response)
    Poll_result.t . For "dispatch" equivalents refer to
...
val send_and_track_rpc_from_poller
  ...
  -> send_rpc:
       ('query
        -> 'underlying Or_error.t Bonsai.Effect_throttling.Poll_result.t Ui_effect.t)
...
val send_and_track_rpc_from_dispatch
  ...
  -> sexp_of_response:('underlying -> Sexp.t) option
  -> path:string
  -> send_rpc:('query -> 'underlying Or_error.t Ui_effect.t)
  -> get_response:('underlying -> 'response)
  -> query:'query
  -> here:Source_code_position.t
